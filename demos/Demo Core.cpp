...
// include haar smile detector
CascadeClassifier smile_cascade(dataPath + "haarcascade_smile.xml");
...
// find smiles in frame
std::vector<Rect> smileDetector(const Mat & img) {
	Mat img_gray;
	cvtColor(img, img_gray, COLOR_BGR2GRAY);
	equalizeHist(img_gray, img_gray);
	std::vector<Rect> res;

	smile_cascade.detectMultiScale(img_gray, res, 2, 30);
